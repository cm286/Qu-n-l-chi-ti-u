...
import { formatVNDSmart, toVN } from '../utils/categoryLabels'
        ...
        {allCategories.map(cat => {
          ...
          return (
            <div key={cat.value} className="bg-gray-50 border rounded-xl p-3 flex flex-col items-center">
              <span className="font-semibold text-gray-700 text-sm">{toVN(cat.value) || cat.label}</span>
              <span className={`text-xs mt-1 font-bold ${remaining > 0 ? 'text-green-600' : remaining === 0 ? 'text-gray-600' : 'text-red-600'}`}>Còn lại: {formatVNDSmart(Math.max(remaining, 0))}</span>
            </div>
          )
        })}
      </div>
